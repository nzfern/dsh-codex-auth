...
class TokenStore {
	...
	async accessToken() {
		return this.read(this.config.accessTokenRef);
	}

	async refreshToken() {
		return this.read(this.config.refreshTokenRef);
	}

	fresh(token) {
		const expiresAt = accessTokenExpiry(token);
		return expiresAt !== undefined && expiresAt - Date.now() > this.config.refreshMarginMs;
	}
	...
	async doRefresh() {
		...
		try {
			const tokens = await refreshTokens(refresh, this.config.clientId);
			await this.write(this.config.accessTokenRef, tokens.access_token);
			if (tokens.refresh_token !== undefined) await this.write(this.config.refreshTokenRef, tokens.refresh_token);
			this.ctx.logger.info("codex-auth: access token refreshed (expires %s)", formatTime(tokens.expiresAt));
			return tokens.access_token;
		} catch (error) {
			if (isInvalidGrant(error)) {
				await this.unset(this.config.accessTokenRef);
				await this.unset(this.config.refreshTokenRef);
...
function apply(ctx, config) {
	...
	if (connection !== undefined) {
		connection.rpc.intercept("/api", (endpoint) => endpoint === "codex.login" || endpoint === "codex.status" || endpoint === "codex.logout", (endpoint, _payload, _signal) => {
			switch (endpoint) {
