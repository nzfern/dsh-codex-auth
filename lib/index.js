...
function apply(ctx, config) {
	...
	ctx.inject(["connection"], (sctx) => {
		sctx.connection.rpc.intercept("/api", (endpoint) => endpoint === "codex.login" || endpoint === "codex.status" || endpoint === "codex.logout" || endpoint === "codex.usage", (endpoint, _payload, _signal) => {
			switch (endpoint) {
				...
				case "codex.usage": return loginService.usage();
