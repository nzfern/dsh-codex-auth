...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function CodexSettingsItem() {
			...
			// Render one WHAM window as a labelled remaining-quota bar.
			const renderWindow = (w, label) => {
				if (w === undefined) return null;
				const remaining = Math.round(100 - w.usedPercent);
				const barColor = remaining <= 5 ? "var(--dsw-alias-state-error-primary, #d33)" : remaining <= 20 ? "var(--dsw-alias-state-warn-primary, #d9822b)" : "var(--dsw-alias-state-success-primary, #10a37f)";
				let resets;
				if (w.resetsAt) {
					const d = new Date(w.resetsAt);
					const pad = (n) => String(n).padStart(2, "0");
					resets = `${pad(d.getMonth() + 1)}-${pad(d.getDate())} ${pad(d.getHours())}:${pad(d.getMinutes())}`;
				}
				return React.createElement("div", { style: { marginTop: "6px" } },
					React.createElement("div", { style: { display: "flex", justifyContent: "space-between", fontSize: "11px", color: "var(--dsw-alias-label-secondary, #888)", marginBottom: "3px" } },
						...
						React.createElement("span", null, `剩余 ${remaining}%${resets ? ` · ${resets} 重置` : ""}`)
					),
					React.createElement("div", { style: { height: "6px", background: "var(--dsw-alias-bg-layer-1, #eef1f4)", borderRadius: "3px", overflow: "hidden" } },
						React.createElement("div", { style: { width: `${Math.min(100, Math.max(0, remaining))}%`, height: "100%", background: barColor, borderRadius: "3px" } })
					)
				);
			};
		...
		function CodexUsageBadge() {
			...
			const used = primary?.usedPercent ?? secondary?.usedPercent ?? 0;
			const remaining = Math.round(100 - used);
			const color = remaining <= 5 ? "var(--dsw-alias-state-error-primary, #d33)" : remaining <= 20 ? "var(--dsw-alias-state-warn-primary, #d9822b)" : "var(--dsw-alias-state-success-primary, #10a37f)";
			const resetsAt = primary?.resetsAt ?? secondary?.resetsAt;
			...
			// Passive floating badge: no click handling at all (pointer-events
			// none), small and unobtrusive, showing remaining quota + reset date.
			return React.createElement("div", { style: { position: "fixed", right: "12px", bottom: "12px", zIndex: 40, fontFamily: "system-ui, sans-serif", fontSize: "11px", lineHeight: 1.5, color: "var(--dsw-alias-label-secondary, #888)", background: "var(--dsw-alias-bg-base, rgba(255,255,255,0.9))", border: "1px solid var(--dsw-alias-border-l1, #e2e2e2)", borderRadius: "8px", padding: "4px 10px", boxShadow: "0 1px 4px rgba(0,0,0,0.08)", pointerEvents: "none", userSelect: "none" } },
				React.createElement("div", { style: { display: "flex", alignItems: "center", gap: "5px" } },
					React.createElement("span", { style: { width: "6px", height: "6px", borderRadius: "50%", background: color, display: "inline-block", flex: "none" } }),
					React.createElement("span", { style: { color: "var(--dsw-alias-label-primary, #555)", fontWeight: 600 } }, `Codex 余量 ${remaining}%`)
				),
				resetText
					? React.createElement("div", { style: { fontSize: "10px", opacity: 0.85, marginTop: "1px" } }, `重置 ${resetText}`)
					: null
			);
		}
