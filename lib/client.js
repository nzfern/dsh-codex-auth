...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Shared small-button style (theme-aware via dsw alias tokens). */
		const buttonStyle = (primary, disabled) => ({
			border: primary ? "none" : "1px solid var(--dsw-alias-border-l1, #ccc)",
			background: primary ? "var(--dsw-alias-button-primary-fill, #10a37f)" : "transparent",
			color: primary ? "var(--dsw-alias-label-inverse, #fff)" : "var(--dsw-alias-label-primary, #555)",
			borderRadius: "6px",
			padding: "6px 14px",
