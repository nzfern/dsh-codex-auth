...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function CodexSidebarButton({ wide }) {
			...
			const [panelPos, setPanelPos] = useState(null); // { left, top } in viewport px

			// Panel geometry: 280px wide, ~250px tall (content can vary, so the
			// vertical estimate only decides which side of the button to open on).
			const PANEL_W = 280;
			const PANEL_H = 250;
			const GAP = 8;
			const MARGIN = 8;

			/** Compute a fixed panel position anchored to the button, flipped to
			 *  stay inside the viewport (prefers above-left of the button). */
			const computePanelPos = useCallback(() => {
				const rect = buttonRef.current?.getBoundingClientRect();
				if (!rect) return { left: 16, top: 64 };
				const vw = window.innerWidth;
				const vh = window.innerHeight;
				// Horizontal: align panel's right edge with the button's right edge,
				// clamped to the viewport.
				let left = rect.right - PANEL_W;
				if (left < MARGIN) left = MARGIN;
				if (left + PANEL_W > vw - MARGIN) left = vw - MARGIN - PANEL_W;
				// Vertical: prefer above the button; flip below when not enough room.
				let top;
				if (rect.top - PANEL_H - GAP >= MARGIN) top = rect.top - PANEL_H - GAP;
				else if (rect.bottom + PANEL_H + GAP <= vh - MARGIN) top = rect.bottom + GAP;
				else top = MARGIN;
				return { left, top };
			}, []);
			...
			// Re-anchor the panel when the sidebar collapses/expands while open.
			useEffect(() => {
				if (!panelOpen) return;
				const reanchor = () => setPanelPos(computePanelPos());
				reanchor();
				window.addEventListener("resize", reanchor);
				return () => window.removeEventListener("resize", reanchor);
			}, [panelOpen, computePanelPos]);
			...
			const togglePanel = useCallback(() => {
				...
				startLogin();
			}, [state.loggedIn, state.loginInProgress, code, panelOpen, openPanel]);
			...
			return React.createElement("div", { style: { position: "relative" } },
				...
				panelOpen && panelPos
					? React.createElement("div", {
						ref: panelRef,
						style: {
							// Fixed at the computed position anchored to the button,
							// always inside the viewport.
							position: "fixed", left: `${panelPos.left}px`, top: `${panelPos.top}px`, zIndex: 50,
							...
							font: "13px system-ui, sans-serif", lineHeight: 1.6, width: "280px", maxWidth: "calc(100vw - 16px)"
						}
					},
