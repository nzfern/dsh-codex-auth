...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const buttonStyle = (primary, disabled) => ({
			border: primary ? "none" : "1px solid var(--dsw-alias-border-l1, #ccc)",
			background: primary ? "#10a37f" : "transparent",
			color: primary ? "#fff" : "var(--dsw-alias-text-primary, #555)",
			borderRadius: "6px",
			padding: "6px 14px",
		...
		function CodexSettingsItem() {
			...
			return React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: "12px", alignItems: "center", justifyContent: "space-between", padding: "12px 0", borderBottom: "1px solid var(--dsw-alias-border-l1, #eee)" } },
				React.createElement("div", { style: { minWidth: "0" } },
					React.createElement("div", { style: { fontWeight: 600, fontSize: "14px" } }, "ChatGPT · Codex 额度"),
					React.createElement("div", { style: { color: "var(--dsw-alias-text-secondary, #888)", fontSize: "12px", marginTop: "2px" } },
						state.loggedIn
				...
				React.createElement("div", { style: { display: "flex", alignItems: "center", gap: "10px" } },
					React.createElement("span", { style: { color: state.loggedIn ? "#10a37f" : "var(--dsw-alias-text-secondary, #888)", fontSize: "12px", fontWeight: 600 } }, statusText),
					state.loggedIn
						? React.createElement("button", { onClick: logout, disabled: busy, style: buttonStyle(false, busy) }, "退出登录")
						: React.createElement("button", { onClick: startLogin, disabled: busy || state.loginInProgress, style: buttonStyle(true, busy || state.loginInProgress) }, "用 ChatGPT 登录")
				),
				state.loginInProgress && code
					? React.createElement("div", { style: { width: "100%", background: "var(--dsw-alias-bg-hover, #f6f8fa)", border: "1px dashed var(--dsw-alias-border-l1, #ccc)", borderRadius: "8px", padding: "10px 12px", marginTop: "4px" } },
						...
						React.createElement("div", { style: { color: "var(--dsw-alias-text-secondary, #888)", fontSize: "12px" } }, "等待授权中(约 15 分钟内有效)…")
					)
					: null,
				state.loginInProgress && state.status === "done"
					? React.createElement("div", { style: { width: "100%", color: "#10a37f", fontWeight: 600, fontSize: "13px" } }, "登录成功!可在模型选择器中选用 Codex 模型。")
					: null,
				state.error
					? React.createElement("div", { style: { width: "100%", color: "#d33", fontSize: "12px" } }, String(state.error))
					: null
			);
		}
